"""Thermal closed-loop endpoint evidence for Figure 6.

Core conclusion:
The continuation rule transfers to a held-out PDE when its numerical evidence
is calibrated for that equation, eliminating the observed unsafe episodes
while preserving task value and reducing certification cost.

Figure archetype:
Asymmetric mixed-modality figure. Panels a-c establish numerical evidence,
cost and time-resolved outcomes. Panel d provides the cohort-level endpoint.

Earlier generators and figure files remain unchanged.
"""
...
OUT_STEM = FIGURE_DIR / "Figure_6_endpoint_panel"


def numerical_levels_panel(ax: plt.Axes) -> None:
    """Show how numerical evidence changes across independently tested levels."""
...
def certification_time_panel(ax: plt.Axes) -> None:
    """Paired certification cost with a clear gutter from panel a."""
...
def load_heldout_episode_peaks() -> tuple[np.ndarray, np.ndarray]:
    rows = [
        row
        for row in source_data.read_csv("SourceData_Fig6_thermal_episodes.csv")
        if row["evaluation_set"] != "validation"
    ]
    raw = np.asarray(
        [float(row["raw_peak_normalized_risk"]) for row in rows],
        dtype=float,
    )
    verified = np.asarray(
        [float(row["verified_continuation_peak_normalized_risk"]) for row in rows],
        dtype=float,
    )
    assert raw.shape == verified.shape == (128,)
    assert int(np.sum(raw > 1.0)) == 6
    assert int(np.sum(verified > 1.0)) == 0
    assert int(np.sum(np.abs(raw - verified) > 1e-12)) == 9
    return raw, verified


def heldout_endpoint_panel(fig: plt.Figure, spec) -> None:
    """Combine all episode-level safety evidence with a compact task summary."""
    raw, verified = load_heldout_episode_peaks()
    ...
    frame.set_axis_off()
    panel_label(frame, "d", x=-0.085, y=1.02)
    frame.text(
        0.02,
        1.02,
        "Held-out cohort",
...
def main() -> None:
    ...
    certification_time_panel(fig.add_subplot(outer[0, 6:]))
    numerical_panels.temporal_risk_panel(fig, outer[1, :8])
    heldout_endpoint_panel(fig, outer[1, 8:])
